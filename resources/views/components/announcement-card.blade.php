...
<a href="{{ route('announcements.show', $announcementId) }}"
    ...
    <div>
        {{-- <div class="flex items-center justify-center">
            <img class="rounded-t-lg" src="{{ $announcementImg }}" alt="{{$announcementTitle}} image" />
        </div> --}}

        <h2 class="mt-6 text-xl font-semibold text-gray-900">{{ $announcementTitle }}</h2>

        {{-- <p class="mt-4 text-gray-500 text-sm leading-relaxed text-ellipsis">
            {{ $announcementDescription }}
        </p> --}}
        <div class="flex space-x-10 align-middle mt-10">
            <ul class="flex space-x-2 self-center">
                {{-- @foreach ($industryFields as $industryField)
                    <li class="text-sm font-semibold text-gray-900 dark:text-white">{{ $industryField->industry_field }}
                    </li>
                @endforeach --}}
            </ul>
            <button type="button"
